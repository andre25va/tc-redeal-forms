...
export default function PdfFillPage({
  ...
  const [mode, setMode] = useState<'chat' | 'manual' | null>(null)
  ...
  // Chat available only if sections defined for this form
  ...
  useEffect(() => {
    const update = () => {
      if (containerRef.current) {
        setDisplayWidth(Math.min(containerRef.current.clientWidth - 8, 800))
      }
    }
    update()
  ...
  useEffect(() => {
    ...
    function tryLoad() {
      if ((window as any).pdfjsLib) {
        if (!libLoadedRef.current) { libLoadedRef.current = true; renderPdf() }
        return
      }
  ...
  const scheduleSave = useCallback((data: Record<string, string>) => {
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        const res = await fetch(`/api/forms/${token}/save`, {
          method: 'POST',
  ...
  // Called by ChatFillPage to batch-update fields
  ...
  const handleSubmit = async () => {
    const msg = language === 'es'
      ? '¿Está seguro de que desea enviar el formulario? No podrá hacer cambios después.'
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-white flex items-center justify-center p-4" style={{ colorScheme: 'light' }}>
        <div className="text-center max-w-md bg-white rounded-2xl shadow-lg p-8">
          ...
          <p className="text-gray-500 mb-1">
            {language === 'es' ? 'Su formulario ha sido enviado exitosamente.' : 'Your form has been submitted successfully.'}
          </p>
          {invitation.property_address && (
            <p className="text-sm text-gray-400 mt-2">{invitation.property_address}</p>
          )}
          <p className="text-sm text-gray-400 mt-3">
            {language === 'es'
              ? `Se enviará una copia completada a ${invitation.seller_email}.`
              : `A completed copy will be emailed to ${invitation.seller_email}.`}
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-gray-100" style={{ colorScheme: 'light' }}>
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <span className="text-xs text-gray-400 hidden sm:block">
                {filledCount} filled · pg {currentPage}/{pageCount}
              </span>
              ...
              <div className="flex rounded-lg overflow-hidden border border-gray-200 text-xs">
                <button
                  ...
                  className={`px-2 py-1 font-medium transition-colors ${language === 'en' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-500 hover:bg-gray-50'}`}
                >EN</button>
                <button
                  onClick={() => setLanguage('es')}
                  className={`px-2 py-1 font-medium transition-colors ${language === 'es' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-500 hover:bg-gray-50'}`}
                >ES</button>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-2 sm:px-4 py-4" ref={containerRef}>
        {pdfLoading ? (
          <div
            className="bg-white shadow-md rounded-sm flex items-center justify-center mx-auto"
            style={{ width: displayWidth, height: pageDisplayH }}
          >
          ...
          <>
            {/* PDF page + overlaid inputs */}
            <div
              className="relative bg-white shadow-md rounded-sm overflow-hidden mx-auto"
              style={{ width: displayWidth, height: pageDisplayH }}
            >
              {pageDataUrl ? (
                <img
                ...
                if (isCb) {
                  return (
                    <input
                      ...
                      style={{ position: 'absolute', left, top, width: w, height: h, cursor: 'pointer', accentColor: '#4f46e5' }}
                    />
                  )
                }
                ...
                return (
                  <input
                    ...
                    style={{
                      ...
                      background: val ? 'rgba(239,246,255,0.85)' : 'rgba(239,246,255,0.45)',
                      border: 'none',
                    ...
                    onBlur={e => {
                      e.currentTarget.style.background = val ? 'rgba(239,246,255,0.85)' : 'rgba(239,246,255,0.45)'
                    }}
                  />
                )
              })}
            ...
            <div className="flex items-center justify-between mt-4 px-1">
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 rounded-xl border border-gray-200 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition shadow-sm"
              >
              ...
              {currentPage < pageCount ? (
                ...
                <button
                  ...
                  disabled={submitting}
                  className="flex items-center gap-2 px-5 py-2 rounded-xl bg-green-600 hover:bg-green-700 text-white text-sm font-bold shadow transition disabled:opacity-60"
                >
            ...
            <p className="text-center text-xs text-gray-400 mt-2">
              {language === 'es' ? 'Su progreso se guarda automáticamente.' : 'Your progress is automatically saved.'}
            </p>
          </>
        )}
      </div>
